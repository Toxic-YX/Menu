...
@interface ViewController ()<YZPullDownMenuDataSource,UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    [self.view insertSubview:self.tableView belowSubview:menu];
    // 设置下拉菜单代理
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:YZUpdateMenuTitleNote object:self userInfo:@{@"arr":titles}];
   [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(click:) name:@"YZUpdateMenuTitleNote" object:nil];
}

- (void)click:(NSNotification *)info {
   
   // 这里请求对应数据, 刷新这个VC中的tableView
   NSLog(@"%@",[info userInfo]);
}
